...
export default function Sidebar({ role, courses, pendingDisputes = 0 }: { role: string, courses?: any[], pendingDisputes?: number }) {
  ...
  return (
    <aside className="w-64 min-h-screen border-r border-border-subtle bg-surface flex flex-col fixed left-0 top-0 overflow-y-auto">
      ...
      <nav className="flex-1 p-4 space-y-2 mt-2">
        ...
        {role === 'lecturer' && courses && courses.length > 0 && (
          <div className="mt-8 animate-in fade-in slide-in-from-left">
              <h3 className="text-xs font-bold text-text-muted uppercase tracking-widest px-4 mb-3">My Assigned Modules</h3>
              {courses.map((c: any) => (
